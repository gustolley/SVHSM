...
# User Input --------------------------------------------------------------
headDir = 'SVHSM_runs/'
scenario_prefix = 'Future_Conditions_'
scenarios = c('Historical', '2030', '2070', '2070DEW', '2070WMW')
...
SW_Inflows_Annual_plot = SW_Inflows_Annual %>%
  ...
  mutate(`Climate Scenario` = factor(`Climate Scenario`, levels = scenarios, ordered = T)) %>%
  ggplot(aes(x = WY, y = `SW Inflow Volume (TAF)`)) +
  geom_line(aes(color = `Climate Scenario`), size = 1) +
  geom_point(aes(fill = `Climate Scenario`, color = `Climate Scenario`), size = 2) +
...
GW_Pumping_Plot = GW_Pumping_Annual %>%
  ...
  scale_color_manual(values = plot_colors, labels = scenarios) + 
  scale_fill_manual(values = plot_colors, labels = scenarios) + 
...
GW_Storage_Cumulative_Annual_Plot = GW_Storage_Cumulative %>%
  pivot_longer(cols = -WY, names_to = 'Climate Scenario', values_to = 'Storage (TAF)') %>%
  mutate(`Climate Scenario` = factor(`Climate Scenario`, levels = scenarios, ordered = T)) %>%
  ggplot(aes(x = WY, y = -`Storage (TAF)`, color = `Climate Scenario`, fill = `Climate Scenario`)) +
  geom_hline(yintercept = 0, color = 'black', size = 0.5) +
  geom_line(size = 0.75) +
  geom_point(size = 1.25) +
  scale_x_continuous(limits = c(WYstart, WYend+1), breaks = seq(WYstart, WYend+1, by = 5), expand = c(0.01,0.01)) +
  scale_y_continuous(limits = c(-80,20), breaks = seq(-80,20, by = 20), expand = c(0,0)) +
  scale_color_manual(values = plot_colors, labels = scenarios) + 
  scale_fill_manual(values = plot_colors, labels = scenarios) + 
...
ggsave(plot = GW_Storage_Cumulative_Annual_Plot, filename = paste0(scenario_prefix,'Cumulative_Storage.png'),
       device = 'png', width = 6.5, height = 4, units = 'in', dpi = 300)
...
Eastside_Storage_Cumulative_Plot = Eastside_Westside_Storage_Cumulative %>%
  ...
  ungroup() %>%
  dplyr::select(-Zone) %>%
  pivot_longer(cols = -WY, names_to = 'Climate Scenario', values_to = 'Storage (acre-ft/day)') %>%
  mutate(`Climate Scenario` = factor(`Climate Scenario`, levels = scenarios, ordered = T)) %>%
  ggplot(aes(x = WY, y = -`Storage (acre-ft/day)`, color = `Climate Scenario`, fill = `Climate Scenario`)) +
  geom_hline(yintercept = 0, color = 'black', size = 0.75) +
  geom_line(size = 0.75) +
  geom_point(size = 1.25) +
  scale_x_continuous(limits = c(WYstart, WYend+1), breaks = seq(WYstart, WYend+1, by = 5), expand = c(0.01,0.01)) +
  scale_y_continuous(limits = c(-200,50), breaks = seq(-200,50, by = 50), expand = c(0,0)) +
  scale_color_manual(values = plot_colors, labels = scenarios) + 
  scale_fill_manual(values = plot_colors, labels = scenarios) + 
...
Eastside_Lower_Storage_Cumulative_Plot = Eastside_Upper_Lower_Storage_Cumulative %>%
  ...
  scale_color_manual(values = plot_colors, labels = scenarios) + 
  scale_fill_manual(values = plot_colors, labels = scenarios) + 
  annotate(geom = 'text', x = 2022, y=-140, label = 'Eastside Lower Aquifer', size = 6, hjust = 0, vjust = 0) +
  ggtitle('Cumulative Change in Storage Rate') +
  ylab('Cumulative Average Daily Rate\n(acre-ft/day)') +
  theme_few() +
  theme_adj +
