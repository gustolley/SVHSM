...
modelDir = paste0('SVHSM_runs/Future_Conditions_',scenario,'/')
...
WYtypesFile = paste0('SVHSM_Simulated_WY_Types_',scenario,'.txt')
resultsDir = paste0(modelDir, 'Results/')


WYstart = 2021                                   # Beginning water year of simulation
WYend = 2070                                     # Ending water year of simulation
NSEGS = 51                                       # Number of SFR Segments
...
WYtypes = read.table(WYtypesFile, header = T) %>%
...
annual_barplot_theme = theme(legend.title=element_blank(),panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
...
SWBM_Annual_Barplot = ggplot(data = NULL) +
  ...
  ggtitle(paste(scenario,'- Annual Soil Water Budget')) +
  geom_rect(data = WYtypes, aes(xmin = plot_annual_start, xmax = plot_annual_end, ymin = SWBM_plot_range[1], ymax = SWBM_plot_range[1] + SWBM_plot_range[2]*0.1),
            fill = WYtypes$plot_color , color = 'black', size = 0.2, show.legend = F) +
  geom_text(data = WYtypes, aes(x = annual_label_date, y = SWBM_plot_range[1]*0.95, label = Type), size = 1.5) +
  # geom_line(data = SWBM_ann_running_storage, aes(x = WY, y = value, color = 'Cumulative Storage'), size = 0.75) +
  # geom_point(data = SWBM_ann_running_storage, aes(x = WY, y = value, color = 'Cumulative Storage'), fill = 'black', size = 1, shape = 21) +
  scale_fill_manual(values = c('skyblue', 'orange', 'gold4','red','navyblue','aquamarine','gray75')) +
  scale_color_manual(values = 'black') +
  annual_barplot_theme +
...
MODFLOW_Annual_Barplot =  ggplot(data = NULL) +
  ...
  ylab('Annual Volume (TAF)') +
  ggtitle(paste(scenario,'- Annual Aquifer Water Budget')) +
  geom_rect(data = WYtypes, aes(xmin = plot_annual_start, xmax = plot_annual_end, ymin = MF_plot_range[1], ymax = MF_plot_range[1] + MF_plot_range[2]*0.1),
            fill = WYtypes$plot_color , color = 'black', size = 0.2, show.legend = F) +
  geom_text(data = WYtypes, aes(x = annual_label_date, y = MF_plot_range[1]*0.95, label = Type), size = 1.5) +
  geom_line(data = MODFLOW_ann_running_storage, aes(x = WY, y = `Flux (TAF)`, color = 'Cumulative Storage'), size = 0.75) +
  geom_point(data = MODFLOW_ann_running_storage, aes(x = WY, y = `Flux (TAF)`, color = 'Cumulative Storage'), fill = 'black', size = 1, shape = 21) +
...
sfr = read.table(paste0(modelDir,'SVHSM.sfr'), skip = 2370, header = F, fill = T) %>% 
  na.omit() %>% 
  mutate(SP = rep(seq(1,length(modelMonths)), each = NSEGS),
         WY = rep(seq(WYstart,WYend), each = NSEGS*12),
         ndays = rep(days_in_month(modelMonths), each = NSEGS),
         total_inflow = 0,
...
SW_Annual_Barplot =  ggplot(data = NULL) +
  ...
  ylab('Annual Volume (TAF)') +
  ggtitle(paste(scenario,'- Annual Surface Water Budget')) +
  geom_rect(data = WYtypes, aes(xmin = plot_annual_start, xmax = plot_annual_end, ymin = SW_plot_range[1], ymax = SW_plot_range[1] + SW_plot_range[2]*0.1),
            fill = WYtypes$plot_color , color = 'black', size = 0.2, show.legend = F) +
  geom_text(data = WYtypes, aes(x = annual_label_date, y = SW_plot_range[1]*0.95, label = Type), size = 1.5) +
...
#   mutate(Volume_TAF = Volume_m3*0.000000810714)
# 
# SWBM_pumping %>% 
...
ZoneBud_Eastside_Westside_Annual_pumping =  ggplot(data = NULL) +
  ...
  ylab('Average Daily Rate (acre-ft)') +
